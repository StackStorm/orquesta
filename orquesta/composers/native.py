...
class WorkflowComposer(base.WorkflowComposer):
    ...
    def _compose_wf_ex_graph(cls, wf_graph):
        ...
        while not q.empty():
            ...
            # For complex multi-level splits and joins, if a task from higher in the hierarchy
            # is processed first, then ignore the task for now. This task will be processed
            # again later in the hierarchy. Otherwise, if this task is processed now, it will
            # be placed in a separate workflow branch.
