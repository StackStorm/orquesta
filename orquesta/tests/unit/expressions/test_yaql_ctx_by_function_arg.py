...
class YAQLVariableExtractionTest(test_base.ExpressionEvaluatorTest):
    @classmethod
    def setUpClass(cls):
        cls.language = "yaql"
    ...
    def test_nested_var_extraction(self):
        expr = "<% ctx(ctx(foobar))  %>"

        expected_vars = ["ctx(ctx(foobar))"]

        self.assertListEqual(expected_vars, self.evaluator.extract_vars(expr))

    def test_nested_var_extraction_with_single_quotes(self):
        expr = "<% ctx(ctx('foobar'))  %>"
    ...
    def test_nested_var_extraction_with_double_quotes(self):
        expr = '<% ctx(ctx("foobar"))  %>'
